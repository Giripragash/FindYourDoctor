...
public class AddPatient extends Activity {
	...
	TextView txt_home_title, txt_addpatient_title;

	EditText ed_pid, ed_name, ed_age, ed_phone, ed_add;
	Spinner spin_sex;
	Button btn_submit;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		txt_addpatient_title = (TextView) findViewById(R.id.txt_addpatient_title);

		txt_home_title.setTypeface(tf);
		txt_addpatient_title.setTypeface(tf);

		ed_pid = (EditText) findViewById(R.id.ed_pid);
		...
		Button butnext = (Button) findViewById(R.id.next_pid_bowser);

		butnext.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				pidname = ed_pid.getText().toString();
				Intent in = new Intent(getApplicationContext(),
						Donorupdation.class);
				in.putExtra("dids", pidname);
				startActivity(in);
		...
		Random r = new Random();
		String rand_number = "p" + r.nextInt(9) + r.nextInt(9) + r.nextInt(9)
				+ r.nextInt(9) + r.nextInt(9);

		ed_pid.setText(rand_number);

		btn_submit.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (!ed_pid.getText().toString().equals("")
	...
	private class AsyncCallWS extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			simpleWaitDialog = ProgressDialog.show(AddPatient.this, "Wait",
					"Connecting...");
		}
	...
	public void call() {
		try {
			String SOAP_ACTION = "http://tempuri.org/addpatientService";
			String METHOD_NAME = "addpatientService";
			String NAMESPACE = "http://tempuri.org/";
			String URL = "http://cyberstudents.in/android/SmartHealthCareService/service.asmx";

			SoapObject request = new SoapObject(NAMESPACE, METHOD_NAME);
			request.addProperty("id", ed_pid.getText().toString());
			...
			if (result.toString().equals("Success")) {
				...
				handler.post(new Runnable() {
					...
					public void run() {
						...
						// Random number generate
						Random r = new Random();
						String rand_number = "p" + r.nextInt(9) + r.nextInt(9)
